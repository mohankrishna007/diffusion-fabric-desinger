...
class PipelineEngine:
    ...
    def _prepare_stage_input(self, stage_number: int, context: PipelineContext) -> StageInput:
        ...
        if stage_number == 0:
            from weaver.diffusion.stages.stage_0_input_acquisition.processor import Stage0Input, RepeatUnit
        ...
        if stage_number == 1:
            from weaver.diffusion.stages.stage_1_canonical_normalization.processor import Stage1Input
            from weaver.diffusion.stages.stage_0_input_acquisition.processor import Stage0Output
        ...
        if stage_number == 2:
            from weaver.diffusion.stages.stage_2_structural_intent.processor import Stage2Input
            from weaver.diffusion.stages.stage_1_canonical_normalization.processor import Stage1Output
            ...
            # Stage 1 always provides file path (source of truth)
            # May also provide in-memory array for small images (performance)
            return Stage2Input(
                pipeline_id=context.pipeline_id,
                stage_number=stage_number,
                metadata={},
                canonical_raster_path=raster.pixel_array_path,  # Always present
                canonical_raster_array=raster.pixel_array,  # Optional (small images)
